...
def fetch_analytics(yt_analytics, video_id):
    ...
    try:
        report = yt_analytics.reports().query(
            ...
            endDate=today,
            metrics="estimatedMinutesWatched,averageViewDuration,audienceRetention,subscribersGained",
...
def save_analytics(sb, video_id, data):
    sb.table("video_analytics").upsert({
        ...
        "audience_retention": data[2],
        "subscribers_gained": data[3]
    }, on_conflict=["video_id", "snapshot_date"]).execute()
